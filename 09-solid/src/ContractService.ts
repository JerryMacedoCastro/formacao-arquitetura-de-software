...
import { saveContractAndInstallments } from "./ContractData.ts";

type Installment = { installmentNumber: number, amount: number, interest: number, amortization: number, balance: number };

export async function saveContract (input: any): Promise<any> {
    ...
    await saveContractAndInstallments(contract);
